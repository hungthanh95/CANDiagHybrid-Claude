...
- **Auto-start measurement.** `VectorCom.__init__` takes `auto_start_measurement: bool = True` and `measurement_start_timeout: float = 10.0`. After `_connect`, `_run` calls `_ensure_measurement_running(app)`: if `app.Measurement.Running` is `False`, it calls `app.Measurement.Start()` and polls `app.Measurement.Running` (pumping `PumpWaitingMessages` between polls, `poll_interval` apart) until `True` or `measurement_start_timeout` elapses, raising `RuntimeError` on timeout (surfaces as a startup failure, same as a failed `_connect`). Set `auto_start_measurement=False` if the measurement is externally managed.
- **Command timeout (`STATUS_ERR_TIMEOUT` / `ERR <seq> 504 ecu_timeout`).** `VectorCom.__init__` takes `cmd_timeout: float = 5.0`. After a command is written to the `Req*` sysvars, `_run` tracks `(seq, kind, sent_at)` as "outstanding" and does not dequeue a new command from `cmd_q` while one is outstanding. If `RspTrigger` hasn't changed within `cmd_timeout` seconds, `_run` pushes `(seq, STATUS_ERR_TIMEOUT, kind, b"")` onto `evt_q` (-> `ERR <seq> 504 ecu_timeout` per the existing `encode_response` mapping) and clears the outstanding state so the next command can be sent. `FakeVectorCom` takes the same `cmd_timeout` (default `0.2`s for fast tests): when `ecu.consume_drop()` is hit, it waits `cmd_timeout` then synthesizes the same `STATUS_ERR_TIMEOUT` event instead of hanging forever.
