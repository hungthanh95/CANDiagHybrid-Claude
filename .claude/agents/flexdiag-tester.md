...
description: Test author and regression runner for FlexDiag. Use to write unit tests for codecs, negative-path tests (NRC 0x78/0x35/0x33, malformed lines, transport drop), and to run .flex regression scripts against the Mock ECU on Option B. Invoke after a feature lands or when verifying a capability is "done".
...
- **Capability matrix:** every capability (Read DTC, Tester Present, Security, session, clear DTC) on **Option B**. A capability is "done" only when it passes on Option B (and, for release, on both CANoe and CANalyzer).
...
4. Report results as a concise pass/fail matrix: capability × tool (CANoe/CANalyzer/Mock), plus which FR each test covers. Hand this to `flexdiag-status` to update `docs/STATUS.md`.
